...
export function renderApp(rootElement: HTMLElement) {
    ...
    const render = () => {
        const {userProfile, sidebarTab, sidebarCollapsed, currentNoteId} = useAppStore.getState();
        ...
        if (!profileExists(userProfile)) {
            ...
            if (sidebarTab === "notes" && currentNoteId) {
                ...
                const noteEditor = ComponentRegistry.createComponent('noteEditor', currentNoteId);
