...
export function createOntologyEditor(): HTMLElement {
  ...
  const importButton = createButton({
    ...
    onClick: async () => {
      const pubkey = prompt(
        "Enter the Nostr public key (npub) of the user whose ontology you want to import:",
      );
      if (pubkey) {
  ...
  const renderTree = (parentId?: string) => {
    ...
    new Sortable(list, {
      ...
      onEnd: (evt) => {
        ...
        if (oldParentId === "root") {
          newOntology.rootIds = newOntology.rootIds.filter(
            (id) => id !== nodeId,
          );
        } else {
