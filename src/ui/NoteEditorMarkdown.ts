...
export function createNoteEditorMarkdown(noteId?: string): HTMLElement {
    ...
    function insertMarkdown(markdown: string) {
        ...
        textarea.setRangeText(insertText, textarea.selectionStart, textarea.selectionEnd, 'end');
        textarea.focus();
        updatePreview();
    }
...
function markdownToHtml(markdown: string): string {
    return markdown
        ...
        // Task lists
        .replace(/^\s*- \[ \] (.*$)/gim, '<input type="checkbox" disabled> $1<br>')
        .replace(/^\s*- \[x\] (.*$)/gim, '<input type="checkbox" checked disabled> $1<br>')
