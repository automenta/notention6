...
export function createNoteEditorMinimal(noteId?: string): HTMLElement {
    ...
    const initialWordCount = initialText.trim().split(/\s+/).filter(word => word.length > 0).length;
...
function createAIMenu(note: Note, textarea: HTMLTextAreaElement, aiService: any): HTMLElement {
    ...
    const expandButton = createButton({
        ...
        onClick: async () => {
            const selectedText = textarea.value.substring(textarea.selectionStart, textarea.selectionEnd) || textarea.value;
            if (selectedText.trim()) {
                const expanded = await aiService.generateText(`Expand on this content: ${selectedText}`);
                if (expanded) {
    ...
    const summarizeButton = createButton({
        ...
        onClick: async () => {
            ...
            if (text.trim()) {
                const summary = await aiService.generateText(`Summarize this content: ${text}`);
                if (summary) {
                    insertTextAtCursor(textarea, `\n\n## Summary\n${summary}`);
                }
            }
    ...
    const improveButton = createButton({
        ...
        onClick: async () => {
            const selectedText = textarea.value.substring(textarea.selectionStart, textarea.selectionEnd) || textarea.value;
            if (selectedText.trim()) {
                const improved = await aiService.generateText(`Improve the writing and clarity of: ${selectedText}`);
                if (improved) {
                    if (textarea.selectionStart !== textarea.selectionEnd) {
                        // Replace selected text
                        const start = textarea.selectionStart;
                        const end = textarea.selectionEnd;
                        textarea.value = textarea.value.substring(0, start) + improved + textarea.value.substring(end);
                    } else {
                        insertTextAtCursor(textarea, `\n\n${improved}`);
                    }
                }
