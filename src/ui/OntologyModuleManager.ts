...
import { OntologyModuleManager, builtInOntologies, OntologyModule } from "../lib/ontologies";
...
export function createOntologyModuleManager(): HTMLElement {
    ...
    subtitle.textContent = "Enhance your note-taking with comprehensive, pre-built ontologies for various domains. Enable the modules you need.";
    ...
    const createStatsSummary = () => {
        ...
        const totalNodes = enabledModules > 0 ? 
            Object.keys(OntologyModuleManager.generateMergedOntology().nodes).length : 0;
    ...
    // Category filter
    const createCategoryFilter = (onCategoryChange: (category: string | null) => void) => {
    ...
    const createModuleCard = (module: OntologyModule) => {
        ...
        checkbox.onchange = () => {
            ...
            if (checkbox.checked) {
                ...
                const moduleOntology = OntologyModuleManager.getModuleAsOntologyTree(module.id);
                if (moduleOntology) {
    ...
    const disableAllButton = createButton({
        ...
        onClick: () => {
            ...
            Object.values(builtInOntologies).forEach(module => {
                Object.keys(module.nodes).forEach(nodeId => builtInNodeIds.add(nodeId));
            });
