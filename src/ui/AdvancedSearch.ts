...
export function createAdvancedSearch(): HTMLElement {
  ...
  function createTagsFilter(): HTMLElement {
    ...
    tagsInput.onchange = () => {
      ...
      setSearchFilters({ ...searchFilters, tags: tags.length > 0 ? tags : undefined });
      performSearch();
    };
  ...
  function createDateFilter(): HTMLElement {
    ...
    startDateInput.onchange = () => {
      setSearchFilters({ ...searchFilters, customDateStart: startDateInput.value || undefined });
      performSearch();
    };
    ...
    endDateInput.onchange = () => {
      setSearchFilters({ ...searchFilters, customDateEnd: endDateInput.value || undefined });
      performSearch();
    };
  ...
  function createValuesFilter(): HTMLElement {
    ...
    if (currentValues) {
      const valueStrings = Object.entries(currentValues).map(([k, v]) => `${k}:${v}`);
    ...
    valuesInput.onchange = () => {
      ...
      if (!valueString) {
        ...
        setSearchFilters({ ...searchFilters, values: Object.keys(values).length > 0 ? values : undefined });
      }
      performSearch();
    };
  ...
  function createResultCard(note: Note): HTMLElement {
    ...
    content.textContent = plainText.length > 150 
