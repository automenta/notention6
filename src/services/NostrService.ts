import {Event, Filter, finalizeEvent, nip04, SimplePool, UnsignedEvent,} from "nostr-tools";
...
export class NostrService {
    ...
    public async publishNote(
        ...
        if (encrypt) {
            ...
            try {
                contentToPublish = nip04.encrypt(
                    this.privateKey,
                    targetPk,
                    note.content,
                );
        ...
        return this.publishEvent(eventKind, contentToPublish, tags, targetRelays);
    }
    ...
    public async decryptMessage(
        ...
        try {
            return nip04.decrypt(this.privateKey, otherPartyPublicKey, payload);
        } catch (error) {
    ...
    public async getEventById(
        ...
        try {
            return await this.pool.get(relaysToUse, {ids: [eventId]}); // Returns the first event found or null
        } catch (error) {
            ...
            async (event) => {
                ...
                for (const localNote of allNotes) {
                    ...
                    const intersection = new Set(
                        [...localTags].filter((t) => remoteTags.has(t)),
                    );
    ...
    public subscribeToDirectMessages(
        ...
        if (!this.isLoggedIn() || !this.publicKey) {
            throw new Error(
                "User not logged in. Cannot subscribe to direct messages.",
            );
        }
